...
class CIG_Ajax_Dashboard {
    ...
    public function get_my_invoices() {
        ...
        if ($filter === 'today') {
            ...
            // Use wp_date for timezone-aware date formatting
            $week_start = wp_date('Y-m-d', strtotime('monday this week', current_time('timestamp'))) . ' 00:00:00';
            ...
            foreach ($items as $it) {
                ...
                if ($exp > $now && $exp <= $threshold) {
                    $expiring[] = [
                        ...
                        'expires_date' => wp_date('Y-m-d H:i:s', $exp),
